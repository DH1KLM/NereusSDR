...
| `src/gui/AboutDialog.cpp` | Lines 106-107: `Contributor{"Jeremy", "KK7GWY", "AetherSDR", "https://github.com/ten9876/AetherSDR"}` — the About-dialog lists AetherSDR in the contributor-table data. | Leave as-is. This is **data** (contributor attribution), not a code-origin claim. Per 25a Flag #11, NereusSDR's AboutDialog.cpp was authored without a formal port header but its design is inspired by AetherSDR's MainWindow about-box. Optional follow-up: add an AetherSDR attribution line to the Copyright block if/when AboutDialog.cpp ever gets the full Phase 4 header treatment. |
