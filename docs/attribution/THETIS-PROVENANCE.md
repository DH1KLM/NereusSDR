...
| NereusSDR-original POD aggregator; single "From Thetis create_rnet defaults" comment cites a default-value origin, not derived code | src/core/codec/CodecContext.h | Lifted from P1/P2 RadioConnection member variables to make codec subclasses pure functions of (ctx × bank); file header declares "NereusSDR-original. No Thetis port; no PROVENANCE row." |
| Asserts BoardCapabilities::preampItemsForBoard() matches Thetis SetComboPreampForHPSDR per-board counts | tests/tst_preamp_combo.cpp | Test fixture cites Thetis console.cs:40755-40825 [@501e3f5] for expected values; no code is derived — the test exercises NereusSDR's BoardCapabilities registry and RxApplet wiring |
