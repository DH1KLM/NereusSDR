# NereusSDR v0.2.3 Alpha Test — OpenHPSDR Radios (P1 & P2)
...
- As of **v0.2.3 (Apr 24, 2026), Phase 3G RX experience + Phase 3P-I antenna integration are feature-complete on top of the Phase 3P hardware userland.** Every Hardware Config page (Antenna-ALEX, OC Outputs, Calibration, HL2 I/O) and every Diagnostics dashboard a Thetis user reaches for has a NereusSDR equivalent; the RX panadapter now carries an AetherSDR-style dBm scale strip; the VFO Flag's DSP grid hosts a full 7-filter Noise Reduction stack (NR1/2/3/4 + DFNR + MNR + ANF) and a cycling Noise Blanker family (NB / NB2 / SNB); Alex antenna routing reaches the radio with full `Alex.cs:310-413` per-band composition including RX-only antennas, RX-Bypass, XVTR, and SKU-driven labels; Linux audio gains a PipeWire-native bridge alongside the pactl fallback. **The Transmit, PA Settings, CAT/Network, Appearance, and Keyboard Setup pages are NOT there yet** — the page tree is built out, but most controls inside those pages are disabled placeholders pending later phases (3M TX, 3J TCI, 3K CAT, 3H skins, etc.). See "Setup pages that look built but are still shell" below for specifics.
- The **goal of your test** is: confirm discovery → connect → listen to a live SSB QSO on your board, plus exercise the **v0.2.3 additions**: dBm scale strip on the right edge of the spectrum, the 3×4 DSP toggle grid on the VFO Flag (NR family + NB cycling + ANF/SNB + right-click DspParamPopups), Alex antenna routing per-band (and RX-only / BYPS / XVTR antennas if your SKU has them), and Linux PipeWire audio (Linux users only).
...
| **3P-I-a** *(new in v0.2.3)* | **Alex antenna integration — core.** AlexController's per-band antenna state now reaches the radio via `RadioConnection::setAntennaRouting`, with three triggers (antennaChanged / bandChanged / Connected) keeping the wire and UI in sync. All five antenna surfaces (VFO Flag, RxApplet, Setup grid, SpectrumOverlayPanel combos, AntennaButtonItem) funnel through AlexController as the single source of truth. HL2 / Atlas hide all antenna UI on `!caps.hasAlex`. New `kPopupMenu` dark stylesheet fixes Ubuntu 25.10 dark-on-dark menus. Closes [#98](https://github.com/boydsoftprez/NereusSDR/issues/98). | Working |
| **3P-I-b** *(new in v0.2.3)* | **RX-only antennas + SKU labels + XVTR.** New `SkuUiProfile` 14-SKU overlay drives per-product RX-only antenna labels (Hermes / ANAN10 → "RX1/RX2/XVTR", ANAN100-class → "EXT2/EXT1/XVTR", 7000D / G2 → "BYPS/EXT1/XVTR"). AlexController gains 6 flags (rxOutOnTx / ext1OutOnTx / ext2OutOnTx mutual-exclusion + rxOutOverride + useTxAntForRx + xvtrActive). P1 bank0 C3 bits 5-7 + P2 Alex0 bits 8-11 wire-locked. Setup → Antenna Control gains RX-only column + 5 TX-bypass checkboxes; Alex-2 Filters tab gates on `caps.hasAlex2`; VFO Flag gains BYPS 3rd button (double-gated on hasRxBypassRelay + hasRxOutOnTx). Full `Alex.cs:310-413 UpdateAlexAntSelection` composition ported (minus MOX coupling + Aries clamp, both deferred to Phase 3M-1). | Working |
| **3G Sub-epic A** *(new in v0.2.3)* | **dBm scale strip on the right edge of the spectrum.** AetherSDR-style port. Wheel inside the strip zooms the dynamic range live; arrow clicks at top/bottom nudge the floor or ceiling and persist back to PanadapterModel; hover shows a cursor crosshair with the dBm value at the pointer; honors the calibration offset. New `DisplayDbmScaleVisible` toggle in Setup → Display lets users hide the strip. | Working |
| **3G Sub-epic B** *(new in v0.2.3)* | **Noise Blanker family port.** Full Thetis NB / NB2 / SNB stack via `NbFamily` wrapper on RxChannel. VFO Flag gets a cycling NB button (Off → NB → NB2 → Off) mirroring Thetis `chkNB` tri-state. RxApplet gains Threshold (0-100) + Lag (0-20 ms) sliders. Setup → DSP → NB/SNB page is now interactive (previously greyed) and wires global defaults via AppSettings. NbMode + full NbTuning struct persist per-slice-per-band; defaults pinned to Thetis `cmaster.c:43-68` byte-for-byte. | Working |
| **3G Sub-epic C-1** *(new in v0.2.3)* | **7-filter Noise Reduction stack.** NR1 (ANR/WDSP), NR2 (EMNR + post2 cascade), NR3 (rnnoise with user-loadable .bin models), NR4 (libspecbleach SBNR), DFNR (DeepFilterNet3 neural NR — cross-platform), MNR (Apple Accelerate MMSE-Wiener — macOS-only), and ANF. Right-clicking any NR button on the VFO Flag opens a `DspParamPopup` with quick-control sliders and a "More Settings…" entry that deep-links into the matching Setup sub-tab. Mutual-exclusion: only one of NR1/NR2/NR3/NR4/DFNR/MNR per slice at a time; ANF stacks. rnnoise + DeepFilterNet3 models bundled into every release artifact. BNR (NVIDIA Broadcast) deferred. | Working |
| **3O Linux PipeWire** *(new in v0.2.3)* | **Native PipeWire audio bridge** alongside the existing pactl / pulse fallback. Live latency telemetry (1 Hz: latency / xruns / process-cb CPU / stream state), separate sidetone / MON sinks, per-slice output routing via `node.name`, lock-free SPSC byte ring on the data thread. New Setup → Audio → Output page (Primary / Sidetone / Monitor cards). AudioBackendStrip header on every Setup → Audio sub-page shows the detected backend + Rescan / Open-logs buttons. `Help → Diagnose audio backend…` opens the same dialog used at first launch when no backend is detected. Optional build dependency: `libpipewire-0.3-dev` ≥ 0.3.50. | Working |
...
| **Post-v0.2.2 maintenance (Apr 22–24)** | **Bench bugs caught on ANAN-G2 after 0.2.2 shipped.** `fix(p2)` initial CmdHighPriority no longer overwrites the persisted DDC2 frequency with a hardcoded 80m seed (audio used to stay silent until you moved the panadapter). `fix(slice)` band-crossing now refreshes VFO Flag / RxApplet antenna labels (the relay was switching but the buttons showed the previous band's antenna). `fix(alex)` `AlexController` per-band antenna selection now persists across app restart (before, picking ANT2 on 20m and quitting reverted to ANT1 on relaunch). `fix(spectrum-overlay)` RX Ant / TX Ant combos in the ANT flyout now actually change the antenna — they were zombie controls with no signal handler. `fix(audio)` AudioEngine fallback honors the requested deviceName + falls back through the QMediaDevices enumeration in order (closes [#112](https://github.com/boydsoftprez/NereusSDR/issues/112)). `fix(vfo)` removed raw deletes that bypassed Qt parent ownership (closes [#113](https://github.com/boydsoftprez/NereusSDR/issues/113)). `fix(theme)` app-wide dark QPalette + baseline QSS bootstrap installed at startup. Plus several PipeWire shakedown fixes on Ubuntu 25.10 (non-blocking ring write, RT-thread qCInfo removal, on_process null-guards, stop()-without-running SEGFAULT). | Working |
| **Inherited from v0.2.2** | All `--profile`, Phase 3P-A through 3P-H, Phase 3O VAX, Phase 3I full P1 family support, attribution enforcement, post-v0.2.1 maintenance fixes (#73 / #69 / #75 / #83 / I2C persistence / FreqCorrectionFactor / OcMatrix lock / RX1 preamp queue / P1 bank ceiling). | Working |

**The short version:** v0.2.3 lands the **Phase 3G RX experience epic** (dBm strip + NB family + 7-filter NR stack) and the **Phase 3P-I Alex antenna integration**. Combined with v0.2.2's Phase 3P hardware userland, **NereusSDR's RX-side feature surface is now broadly comparable to Thetis for the operating bands and modes most hams use day-to-day.** Every Hardware Config + Diagnostics page a Thetis user reaches for has a NereusSDR equivalent; every PA / connection / settings-validation readout a Thetis user expects is exposed; the DSP grid hosts a full NR + NB family. **Much of the rest of the Setup tree is still NOT there yet.** The Transmit section, PA Settings, CAT/Network, Appearance, Keyboard, and parts of Diagnostics and Display remain page shells with **disabled placeholder controls** — scaffolding for later phases (3M, 3J, 3K, 3H). Single-receiver RX works end-to-end. **TX pipeline (Phase 3M) is still cold**; multi-panadapter (3F), TCI (3J), CAT (3K), skins (3H), recording (3M) remain not-started.
...
### 12. dBm scale strip (new in v0.2.3, Phase 3G Sub-epic A)

The right edge of the spectrum widget now hosts a vertical dBm scale strip ported from AetherSDR.

1. With a live signal, **scroll the mouse wheel inside the strip** (not over the spectrum body). The dynamic range zooms live — the floor stays put while the ceiling moves with the wheel direction, or vice versa depending on which half of the strip you're hovering. Release zoom limit values should persist back to PanadapterModel after a brief settling period.
2. **Click the up/down arrows at the very top and bottom of the strip.** Each click nudges the floor or ceiling by a fixed dB step. Confirm the spectrum/waterfall colors and the dB grid labels respond live.
3. **Hover anywhere on the strip.** A horizontal cursor crosshair should appear with the dBm value at the pointer position.
4. Open **Setup → Display → Spectrum** and toggle **"Show dBm scale strip"**. The strip should hide / reappear instantly. The toggle should round-trip across app restart.
5. If you have a Calibration → TX Display Cal offset configured, on-strip labels should reflect the calibrated dBm — not the raw FFT bin value. (Not testable without a known reference signal; just confirm the toggle and the strip don't crash on connect.)

**✅ Success bar for step 12:** wheel zooms live; arrow clicks nudge the bounds; hover shows the cursor crosshair; the toggle hides/shows it cleanly.

### 13. DSP toggle grid + NR family (new in v0.2.3, Phase 3G Sub-epic B + C-1)

The VFO Flag's DSP tab now hosts a 3×4 toggle grid mirroring AetherSDR.

1. Open the VFO Flag's **DSP** tab. You should see 12 buttons in a 3×4 grid: a **NB** cycling button (left column) and the NR family (NR1 / NR2 / NR3 / NR4 / DFNR / MNR), plus **ANF** and **SNB** independent toggles.
2. **Click NB.** It should cycle: Off → NB → NB2 → Off. Audio character should change with each click on a noisy band.
3. **Click each NR button in turn** (NR1 → NR2 → NR3 → NR4 → DFNR → MNR — macOS only for MNR). Only one of these can be active at a time (mutual exclusion); clicking a different one should cancel the previous. Audio character should change perceptibly on each — DFNR (DeepFilterNet3 neural NR) is the most aggressive, NR1 the most conservative.
4. **Right-click any NR button** (e.g. NR2). A `DspParamPopup` should appear with 3-6 quick-control sliders (Gain Method / NPE Method / AE Filter / Position for NR2; Reduction / Smoothing / Whitening / Rescale / PostThresh / Algo for NR4; Strength / Aggressiveness / Floor for MNR; AttenLimit / PostFilterBeta for DFNR). Move a slider; you should hear the change.
5. Click **"More Settings…"** in the popup. It should deep-link into Setup → DSP → NR/ANF on the matching sub-tab.
6. **Click ANF.** It's independent of the NR mutex — should activate alongside whichever NR is on, and the carrier-notch behavior on a het tone should change.
7. **Tune to another band, then back.** Per-slice-per-band persistence: NR + NB state on each band should restore from where you left it.

**Known limits:**
- **NR3 model load:** if you've never run `Setup → DSP → NR/ANF → NR3` before, the rnnoise default model will load from the bundled `Default_large.bin`. Pointing it at a custom .bin should work but is alpha-tested only for the default model.
- **DFNR:** requires the bundled `DeepFilterNet3_onnx.tar.gz` model. If you built locally without running `setup-deepfilter.sh`, the DFNR button will be disabled.
- **MNR:** macOS only. The button will be disabled on Linux / Windows.
- **BNR:** intentionally absent. NVIDIA Broadcast NR (BNR) requires a gRPC/NIM transport that's out of scope for v0.2.3.

**✅ Success bar for step 13:** NB cycles tri-state; each NR (those available on your platform) audibly changes the audio; right-click opens a settings popup; "More Settings…" jumps into Setup; per-band state persists across band changes.

### 14. Alex antenna routing per-band (new in v0.2.3, Phase 3P-I-a + 3P-I-b)

If your radio has an Alex board (most ANAN family), antenna routing is now reachable from five places, all routed through a single AlexController.

1. Open **Setup → Antenna / ALEX → Antenna Control**. Set per-band TX and RX1 antennas across several bands (e.g. ANT1 on 80m, ANT2 on 40m, ANT3 on 20m).
2. Tune the VFO across those band boundaries. The TX / RX antenna labels on the **VFO Flag**, **RxApplet** ANT row, and **AntennaButtonItem** (in the meter container, if you have one) should all change as you cross each boundary. Behind the scenes the radio is being told to switch the relay live.
3. **Open the Spectrum overlay → ANT flyout.** RX Ant / TX Ant combos should reflect the current band's selection. Change one — the same triple of UI surfaces should update, AND the change should persist back through to Setup → Antenna Control's grid.
4. Quit and relaunch. Per-band antenna selections should restore exactly. *(Verifies the v0.2.2-shipped persistence bug fix.)*

**If your radio is a 7000DLE / 8000DLE / G2 / G2-1K / 200D (boards with `caps.hasRxBypassRelay`):**
5. The VFO Flag's antenna row should show **3** buttons: blue RX, grey **BYPS**, red TX. Click BYPS to toggle RX-Bypass on TX. The `Setup → Antenna Control → "RX Bypass on TX"` checkbox should sync bidirectionally.
6. Open **Setup → Antenna Control**. The RX-only column should be present (3 dropdowns: typically labeled "BYPS / EXT1 / XVTR" on G2-class boards). Set per-band RX-only antennas. The wire emits these via P1 bank0 C3 bits 5-7 or P2 Alex0 bits 8-11.

**If your radio is HL2 or Atlas (`!caps.hasAlex`):**
- The VFO Flag, RxApplet, SpectrumOverlayPanel, and AntennaButtonItem antenna UI should all be **hidden**. You'll see no zombie ANT2/ANT3 buttons. (HL2/Atlas has no Alex relay so the controls would write nothing.)

**If your radio is an HPSDR-kit Apollo board:**
- Setup → Hardware → Apollo PA / ATU / LPF checkboxes should be enabled. Every other ANAN board: Apollo controls should be disabled (every ANAN explicitly disables Apollo in Thetis).
...
If you're on a recent Linux system (Ubuntu 24.04+, Fedora 39+, Arch — anything PipeWire-default) and you installed `libpipewire-0.3-dev` ≥ 0.3.50 before building, NereusSDR will now use the PipeWire-native bridge instead of the pactl fallback.

1. Launch NereusSDR. If no audio backend is detected (rare — PipeWire is detected automatically), the **VaxLinuxFirstRunDialog** should appear with copy-pasteable install instructions for your distro and a Rescan button.
2. Open **Setup → Audio**. There should be an **AudioBackendStrip** header at the top of every Audio sub-page showing the detected backend (`PipeWire` / `Pactl` / `None`) plus Rescan and Open-logs buttons.
3. Open **Setup → Audio → Output**. Confirm three cards: Primary (your speakers), Sidetone, Monitor. Per-slice routing section below them. Live telemetry: latency / xruns / process-cb CPU / stream state should update at ~1 Hz.
4. Open **Setup → Audio → VAX**. VAX is now a virtual source the system sees, not a device the user picks. Open `pavucontrol` (or your PipeWire equivalent) and confirm the `nereussdr-vax-*` source(s) show up as recordable inputs that other apps (WSJT-X, Audacity) can read.
5. **Help → Diagnose audio backend…** opens the same first-launch dialog. Useful when a backend disappears mid-session (e.g. you stopped the PipeWire user service).
