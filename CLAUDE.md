...
Current version: **0.3.2** (set in `CMakeLists.txt`; tagged pre-releases use `vX.Y.Z-rcN` suffix).
...
### Current Phase: post-v0.3.2 (shipped 2026-05-05) — work in flight toward v0.3.3 / v0.4.0.

**Work landed on `main` between v0.3.2 release and 2026-05-08 (~22 commits / 5 PRs):**

* **3M-3a-iv anti-VOX cancellation feed.** Closed the gap from 3M-3a-iii where the gain control was plumbed end-to-end but `SendAntiVOXData` was never called. 4 new WDSP wrappers, `RxDspWorker` → `TxWorkerThread` → `TxChannel::sendAntiVoxData` per-chunk pump (single-RX direct path; aamix port deferred to 3F multi-pan). Setup → Transmit → DEXP/VOX gains `grpAntiVOX` parity (`chkAntiVoxEnable` / `udAntiVoxGain` / `udAntiVoxTau` with verbatim Thetis tooltips). Architectural divergence from Thetis: source-selector (`chkAntiVoxSource` RX vs VAC) is dropped — VAX is a digital-mode app bus with no mic-feedback path, so the audio-output device is the only valid cancellation reference. Bench-verification matrix at `docs/architecture/phase3m-3a-iv-verification/README.md`.
* **Live-apply infrastructure (PRs #219 + #221).** `WdspEngine::rebuildRx/TxChannel` + `RadioModel::setSampleRateLive` + `setActiveRxCountLive`. The codex-P2 follow-up wired the Radio Info combo to the live-apply path. **Crashed on first combo-change** (post-v0.3.2 hot-fix landed 2026-05-08, see below). Active-RX-count infra stays in tree; sample-rate path was rewritten.
* **AF Gain rewire (PR #218, KM4BLG).** Routes the AF slider through `WDSP.SetRXAPanelGain1` instead of fighting the master-volume atomic post-DSP. Closes a long-standing distortion bug (`panel.gain1=4.0` default leaked +12 dB silently). Side effect: VAX inherited the same +12 dB pre-fix and felt "really hot" — VAX bypass landed 2026-05-08 to restore it.
* **Step-att MOX clobber (PR #220, issue #200).** RX step-attenuator value survives an MOX cycle. Closes a regression where the TX-side ATT-on-TX safety scaffolding was over-writing the persisted RX ATT.
* **HL2 FPGA temperature** on bottom banner (commit f50f29d).
* **PureSignal hardening on HL2.** AutoAtt convergence + disconnect crash fix (f993041), initial PS timing parameters pushed to WDSP at init (447faf5), duplicate audio-volume listener dropped + `Default SetPk` routed via `setHwPeak` (fc28cdf), `correctingChanged` split into a dedicated `correctionsBeingAppliedChanged` signal (b7fafaa), `psEnabledChanged` fan-out from the cmd-state machine (e5114f2), single-cal retry loop in `StayOn` (c18e85d), TINT setter wired through `SetPSIntsAndSpi` (75b6a35).
* **Spectrum polish batch (4651e51 + 7bebacb + c966750).** Cursor-freq-display unification, `dispNormalize` ported, peak/binwidth GPU paths, grid band header, replan crossfade, source-first port of Thetis `processNoiseFloor` for the NF overlay, Hz/bin auto-zoom override, NF-tracking dialog-reopen restore.
* **DSP correctness (bed0bd2).** Deterministic `onModeChanged` contract + UB-free test setup.
* **Compliance + cite touchups.** `//MW0LGE` wave-recorder tag preserved in AF-Gain port (c1b7f5a), license markers aggregated in `wdsp_api.h` header window (96601eb), provenance row dropped for retired test (baad89c), `[v2.10.3.14]` stamps on two new Thetis cites (96a33ec), PSpeak_TextChanged cite line range corrected (67298ff), `//MI0BOT` + `//MW0LGE` tags preserved near `PSForm.cs:760/772` (9cb68da).

**Uncommitted work staged 2026-05-08 (in worktree, not yet pushed):**

1. **Sample-rate-live crash hot-fix.** PR #219's destroy-and-recreate path replaced with the Thetis-faithful `SetXcmInrate` path. New `RxChannel::setSampleRate` (carry-only state mutation) + `WdspEngine::setRxChannelRate` (`SetInputSamplerate` + `SetInputBuffsize` on the live channel — `cmaster.c:453-507 [v2.10.3.13]`). `RadioModel::setSampleRateLive` rewritten as the 12-step Thetis sequence ported from `setup.cs:7003-7159 [v2.10.3.13]`. TX channel intentionally untouched (Thetis `audio.cs:637-672 [v2.10.3.13]` routes `SetXcmInrate(0|1)` for RX1/RX2 only). New test `tst_set_sample_rate_live.cpp` (7 cases, all green); 19/19 pre-existing `tst_sample_rate_catalog` still green.
2. **HL2 sample-rate parity (`mi0bot-Thetis setup.cs:849-851 [v2.10.3.13]`).** HL2 now exposes 48k / 96k / 192k / **384k** on P1, matching mi0bot. NereusSDR previously dropped HL2 to the 3-rate base list. Renamed `kP1RatesRedPitaya` → `kP1RatesWithExtra384k` since two boards share the same rate list now. New test `p1_hermes_lite_gets_extra_384`.
3. **Setup → Hardware styling.** `applyComboStyle` extended to put `spin-down.svg` on the dropdown arrow, new `SpinBoxStyle.h` sibling helper (per-widget SVG up/down arrows), 5 combos and 1 spinbox across `RadioInfoTab` / `DiversityTab` / `OcOutputsHfTab` styled + width-capped at 160 px so Setup combos stop stretching to dialog width.
4. **Active RX count widget removed** from Radio Info tab (was disabled-and-stuck-at-1 in single-RX builds). The underlying `RadioModel::setActiveRxCountLive` coordinator stays wired; widget re-exposes when 3F lands. Copy Support Info button repositioned to left-aligned natural-width.
5. **VAX AF-Gain bypass.** `AudioEngine::rxBlockReady` inverse-scales VAX-tap samples by `1/afGain` so the digital-mode bus level stays calibrated regardless of where the user puts the speaker AF slider. Restores the pre-PR-#218 "really hot" baseline that WSJT-X and similar apps depend on. Edge case: AF=0 (full mute) → VAX silent; full decoupling needs a pre-`PanelGain1` WDSP tap (deferred).

**Prior context (v0.3.1, shipped 2026-05-03):** 3M-1 SSB TX (PR #152), Phase 3Q connection workflow refactor + chrome layer (PR #158), 3M-3a-i TX EQ + Leveler + ALC, 3M-3a-ii CFC + CPDR + CESSB + Phase Rotator (MicProfileManager 91 keys, 19/21 factory profiles ported verbatim), ParametricEq widget port (PR #159), Plan 4 ui-polish-foundation epic (PR #166). HL2 ATT/filter safety audit closed. Pre-emphasis remains de-scoped from 3M-3a-ii to 3M-3b (FM-mode follow-up).

**Next on the TX epic: 3M-2 CW TX** — sidetone, firmware keyer, QSK/break-in. Absorbs the HL2 CWX bit-3 follow-up (`networkproto1.c:1247-1252 [@c26a8a4]`; desk-review B3, "HL2 firmware uses bit 3 of I-low byte for CWX PTT, non-HL2 boards don't"). After 3M-2: 3M-3b (FM pre-emphasis), 3M-4 PureSignal, then 3F multi-panadapter and the longer-tail 3H / 3J / 3K phases.
